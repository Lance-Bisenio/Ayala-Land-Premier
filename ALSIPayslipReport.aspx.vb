...
Partial Class ALSIPayslipReport
	Inherits System.Web.UI.Page
	Dim vSQL As String = ""
	Public Earnings As String = ""
	Public Deduction As String = ""

	Private Sub PayslipReport_Load(sender As Object, e As EventArgs) Handles Me.Load

		'Session("uid") = "2022-0080"
		...
		If Not IsPostBack Then
			...
			vSQL = "select Count(User_Id) as Ctr from Tblrightslist where Property_Value='3000' and User_Id='" & Session("uid") & "'"
			...
			'BuildCombo("select EmployeeCode, FullName from tblEmployees where Active=1 and DateSeparated is null order by FullName", CmdEmployeeList)
	...
	Private Sub GetPayrollRunList()
		...
		vSQL = "select BatchNo,PayrollPeriod, FORMAT(PayDate, 'MM/dd/yyyy') as PayDate,FileNameRecurring,FileNameOneTime,Remarks,CreatedBy,DateCreated,PostedBy,DatePosted " _
			...
			& "and Year(DateCreated)<'2024' and Month(DateCreated)<'10' and BatchNo in (select BatchNo from tblPayrollSummary where EmpCode='" & Session("EmpId") & "') " _
	...
	Private Sub Get2307List()
		...
		vSQL = "select Periodfrom, Periodto, BatchNumber, year(Periodfrom), Month(Periodfrom) from tbl2307 " _
				& "where EmployeeCode='" & Session("EmpId") & "' and Year(DateCreated) < '2024' and IsPosted=1 order by year(Periodfrom), Month(Periodfrom) "
		'Response.Write(vSQL)
		da = New SqlClient.SqlDataAdapter(vSQL, c)
	...
	Private Sub Tbl2307_SelectedIndexChanged(sender As Object, e As EventArgs) Handles Tbl2307.SelectedIndexChanged
		...
		vSQL = "select AutoFileName from tbl2307 where BatchNumber='" & Tbl2307.SelectedRow.Cells(1).Text & "' and EmployeeCode='" & Session("uid") & "'"
		...
		Response.Redirect("~/Downloads/ALSI2307/" & Filename & ".pdf")
	End Sub
